...
flags.DEFINE_string('camera_traj', None, 'rendering camera trajectory directory')
flags.DEFINE_string('camera_latent', None, 'txt file of latent to render (idx)')
flags.DEFINE_string('video_dir', None, 'where to save video (with video name)')
FLAGS = flags.FLAGS
...
# generated
def render_scene(rng, devices, test_cameras, render_fn, state, datasource, render_dir, video_dir, latents, fps = 24):
  ...
  for i in tqdm(range(len(test_cameras))):
    ...
    # select latents
    latent_id = str(int(latents[i])).zfill(5)
    appearance_id = datasource.get_appearance_id(latent_id)
    warp_id = datasource.get_warp_id(latent_id)
    batch['metadata'] = {
        'appearance': appearance_id*jnp.ones_like(batch['origins'][..., 0, jnp.newaxis], jnp.uint32),
        'warp': warp_id*jnp.ones_like(batch['origins'][..., 0, jnp.newaxis], jnp.uint32),
    }
    ...
    rgb_img_name = str(i).zfill(5) + '_rgb.png'
    depth_img_name = str(i).zfill(5) + '_depth.png'
  ...
  # generate videos 
  print("generate video at {} with fps : {}\n".format(video_dir, fps))
  mediapy.write_video(video_dir, images, fps=fps)
  print("Done!")
...
def main(argv):
  ...
  del argv
  # camera path
  camera_traj = FLAGS.camera_traj
  # latent index 
  camera_latent = FLAGS.camera_latent
  ...
  #############################################################################
  # modified parts (for rendering)

  # load camera
  camera_dir = Path(camera_traj)
  ...
  # load caemra latent
  with open(camera_latent, "r") as ff:
    ff_content = ff.read()
    latents = ff_content.split(",")
